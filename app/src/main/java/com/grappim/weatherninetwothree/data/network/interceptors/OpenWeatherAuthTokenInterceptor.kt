...
@Singleton
class OpenWeatherAuthTokenInterceptor @Inject constructor(
    ...
    private fun getAnnotation(request: Request): RequestWithOneWeatherApi? =
        request.tag(Invocation::class.java)
            ?.method()?.getAnnotation(RequestWithOneWeatherApi::class.java)

    private fun Request.putAppIdQuery(): HttpUrl =
        this.url.newBuilder().addQueryParameter(
            APP_ID,
            appBuildConfigProvider.openWeatherApiKey
        ).build()
